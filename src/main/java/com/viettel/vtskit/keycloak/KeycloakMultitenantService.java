...
public class KeycloakMultitenantService {
    ...
    /**
     * change user password for specific realm
     * @param realmName
     * @return UserResourceResponse
     */
    public void changeUserPassword(String realmName, String userId, String password){
        validateEnableAdminOperation();
        CredentialRepresentation credentials = new CredentialRepresentation();
        credentials.setType(CredentialRepresentation.PASSWORD);
        credentials.setValue(password);
        credentials.setTemporary(false);
        kcAdminClient.realm(realmName).users().get(userId).resetPassword(credentials);
        kcAdminClient.realm(realmName).users().get(userId).logout();
    }
     ...
     * @param userId
     */
    public ClientMappingsRepresentation getUserClientRole(String realmName, String userId){
        UserResource userResource=kcAdminClient.realm(realmName).users().get(userId);
        ClientMappingsRepresentation roleRepresentationList = userResource.roles().getAll().getClientMappings().get(this.keycloakProperties.getResource());
        return roleRepresentationList;
    }

    /**
     * get Role of User ( realm role ) for specific realm
     * @param realmName
     * @param userId
     */
    public List<RoleRepresentation> getUserRealmRole(String realmName, String userId){
        UserResource userResource=kcAdminClient.realm(realmName).users().get(userId);
        List<RoleRepresentation> roleRepresentationList = userResource.roles().getAll().getRealmMappings();
        return roleRepresentationList;
    }

    public List<GroupRepresentation> getUserGroup(String realmName, String userId){
        UserResource userResource=kcAdminClient.realm(realmName).users().get(userId);
        List<GroupRepresentation> userGroup = userResource.groups();
        return userGroup;
    }
